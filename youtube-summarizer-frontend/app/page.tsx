...
export default function YouTubeSummarizer() {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      alert("Video summarized successfully!")
    } catch (err) {
      clearInterval(progressInterval)
      setError(
  ...
  const downloadPDF = () => {
    if (result?.summary) {
      ...
      const lines = doc.splitTextToSize(result.summary, 180) // max line width
      doc.text(lines, 10, 10)

      // Save the PDF
      doc.save(`youtube-summary-${result.video_details.video_id}.pdf`)
